  ...
  (
    ...
    React.useEffect(() => {
      ...
      corners.forEach(([x, y]) => {
        ...
        ctx.fillStyle = bgColor;
        ctx.fillRect(x + cellSize, y + cellSize, cornerSize - 2 * cellSize, cornerSize - 2 * cellSize);
        ctx.fillStyle = fgColor;
        ctx.fillRect(x + 2 * cellSize, y + 2 * cellSize, cornerSize - 4 * cellSize, cornerSize - 4 * cellSize);
      });
      ...
      if (imageSettings) {
        ...
        img.onload = () => {
          ...
          if (imageSettings.excavate) {
            ctx.fillStyle = bgColor;
            ctx.fillRect(imgX - 5, imgY - 5, imageSettings.width + 10, imageSettings.height + 10);
          }
          
          ctx.drawImage(img, imgX, imgY, imageSettings.width, imageSettings.height);
        };
