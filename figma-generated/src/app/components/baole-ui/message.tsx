...
const Message = React.forwardRef<HTMLDivElement, MessageProps>(
  ({ type = "info", content, duration = 3000, onClose, icon, className }, ref) => {
    React.useEffect(() => {
      if (duration > 0) {
    ...
    return (
      ...
      >
        {icon || icons[type]}
        <span className="flex-1 text-sm text-[var(--color-text)]">{content}</span>
        {onClose && (
          <button
            onClick={onClose}
...
const getContainer = (): HTMLDivElement => {
  if (!messageContainer) {
    ...
    messageContainer.className = "fixed top-4 left-1/2 -translate-x-1/2 z-[10000] flex flex-col gap-2";
